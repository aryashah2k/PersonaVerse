...
class User(UserMixin):
    ...
    def get_credits(self):
        return user_db.get_credits(self.username)
...
# Decorator to check credits before execution
def requires_credits(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        ...
        if credits <= 0:
            return jsonify({'error': 'Insufficient credits. You need at least 1 credit to execute surveys.'}), 403
...
def samplespace():
    history = get_upload_history()
    current_filename = config_manager._current_filename
    return render_template('samplespace.html', history=history, user=current_user, current_filename=current_filename)
...
def execute():
    history = get_upload_history()
    return render_template('execution.html', history=history, user=current_user)
...
def get_execution_metrics():
    try:
        ...
        # Get provider for frontend display
        provider = json_processing.get_json_nested_value(config, "llm_settings.provider")
        ...
        return jsonify({
            ...
            'provider': provider if provider != "not found" else None
        })
...
def start_execution():
    try:
        # Deduct one credit before execution
        success, remaining_credits = user_db.deduct_credit(current_user.username)
        if not success:
            return jsonify({
                'success': False,
