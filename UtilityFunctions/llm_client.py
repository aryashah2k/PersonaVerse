...
class LLMClient:
    ...
    def _setup_client(self):
        ...
        self.rate_limit_delay = llm_settings.get("rate_limit_delay", 1.0)  # seconds between requests
    ...
    def _apply_rate_limit(self):
        """Apply rate limiting for Groq API to prevent abuse"""
    ...
    def generate_multimodal(self, file_path: str, prompt: str, system_prompt: Optional[str] = None,) -> str:
        """
        Upload a file to OpenAI/Anthropic and generate a response using multimodal processing.
        ...
        NOTE: Groq does not support multimodal processing - only OpenAI and Anthropic.
        """
        ...
        if self.provider == "groq":
            raise ValueError(
                "Groq does not support multimodal (PDF/image) processing. "
                "Please switch to OpenAI or Anthropic in Settings, or convert your PDF to plain text."
            )
