...
def questionnaire_iterator_segment(config_set, processed_data, question_segments, execution_order, sample_space, sample_space_size, sample_dimensions, upload = False, progress_file=None, multi_modal=False):
    ...
    for agent_id in range(sample_space_size):
        ...
        while current_question <= survey_size:
            ...
            format_part = """CRITICAL: Your response must contain ONLY valid JSON format, nothing else. Do not include any explanations, reasoning, or additional text before or after the JSON. The output format should be in JSON, with each value structured as "question number": {"reasoning": "explanation of why you chose this answer based on your profile", "answer": "your answer"}. You MUST provide reasoning for every answer explaining why you made that choice based on your persona profile. Do not place ```json at the beginning or end. Start your response directly with { and end with }. No other text is allowed."""
            if multi_modal:
...
def questionnaire_iterator(config_set, processed_data, execution_order, sample_space, sample_space_size, sample_dimensions, upload = False, progress_file=None, multi_modal=False):
    ...
    for agent_id in range(sample_space_size):
        ...
        The output format should be in JSON, with each value structured as "question number": {"reasoning": "explanation of why you chose this answer based on your profile", "answer": "your answer"}.
        You MUST provide reasoning for every answer explaining why you made that choice based on your persona profile.
