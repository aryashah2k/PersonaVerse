...
def cost_estimation(config_set, processed_data, question_segments, sample_space_size, sample_profile_0, output_max_token = 256):
    ...
    Returns:
        float: Estimated total cost in USD, or -1 if model pricing not found, or 0 for free providers (Groq)
    """
    ...
    # Check if provider is Groq (free API with rate limits)
    if provider and provider.lower() == "groq":
        logger.info(f"Groq provider detected - API is free (rate-limited). Model: {model_name}")
        logger.info(f"Estimated tokens - Input: {input_token_estimation if 'input_token_estimation' in locals() else 'N/A'}, Output: {output_token_estimation if 'output_token_estimation' in locals() else 'N/A'}")
