...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ responseUrl, onStartNew }) => {
  const handleDownload = () => {
    historyService.downloadFile(responseUrl);
  };

  const fileType = getFileTypeFromUrl(responseUrl);
        ...
        <Card sx={{ mb: 4, p: 2, textAlign: 'left' }}>
          <CardContent sx={{ p: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              <FilePresentIcon fontSize="large" color="primary" sx={{ mr: 2 }} />
              <Box>
        ...
        <Box sx={{ mt: 4 }}>
          <Alert severity="info" sx={{ textAlign: 'left' }}>
            <Typography variant="body2">
              ...
              <Link href="/history" underline="hover">history page</Link>.
            </Typography>
          </Alert>
        </Box>
      </ResultCard>
